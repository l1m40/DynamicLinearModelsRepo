...
NULL %>% 
  rbind(result_basic_df %>% summarise(error=mean(abs((forecast/price)-1),na.rm=T)) %>% mutate(test="basic estimate")) %>% 
  rbind(result_mle_df   %>% summarise(error=mean(abs((forecast/price)-1),na.rm=T)) %>% mutate(test="maximum likelihood"))
...
result_mle_df   <- price_forecast_mle(Y_df %>% filter(open>0) %>% mutate(price=(price+high+low)/3),80,build)
...
# dlmFilterDF ##############################################
plot_smoothed_dlmFilterDF <- function(df){
  df %>% mutate(out_ci=(price>forecast+ci | price<forecast-ci)) %>% 
    ggplot(aes(x=date))+
    # geom_ribbon(aes(ymax=(forecast+sd_residuals*qnorm(0.95)),ymin=(forecast-sd_residuals*qnorm(0.95))),alpha=.1,fill="cyan3")+
    geom_ribbon(aes(ymax=(forecast+ci),ymin=(forecast-ci)),alpha=.1,fill="cyan3")+
    geom_line(data=Y_df,aes(y=price),color="gray")+
    geom_line(aes(y=price   ),color="black")+geom_point(aes(y=price    ,color=out_ci),size=.5)+scale_color_manual(values=c("TRUE"="red","FALSE"="black"))+
    geom_line(aes(y=forecast),color="cyan3")+geom_point(aes(y=forecast),color="cyan3",size=.5)+
    theme_minimal()
}
Y_df <- tq_get("ITUB4.SA",from=today()-300,to=today()+1,get="stock.prices") %>% mutate(price=adjusted,asset=str_remove(symbol,".SA")) %>% mutate(adjust_gap=close-adjusted,open=open-adjust_gap,high=high-adjust_gap,low=low-adjust_gap)
...
result_dlmFilterDF_df         <- price_forecast_dlmFilterDF(Y_df,80)
result_dlmFilterDF_typical_df <- price_forecast_dlmFilterDF(Y_df %>% filter(open>0) %>% mutate(price=(price+high+low)/3),80)

NULL %>% 
  rbind(result_dlmFilterDF_df         %>% select(forecast,price) %>% mutate(error=abs((forecast/price)-1)) %>% mutate(err_mean=mean(error,na.rm=T),err_max=max(error,na.rm=T)) %>% mutate(test="dlmFilterDF")) %>% 
  rbind(result_dlmFilterDF_typical_df %>% select(forecast,price) %>% mutate(error=abs((forecast/price)-1)) %>% mutate(err_mean=mean(error,na.rm=T),err_max=max(error,na.rm=T)) %>% mutate(test="dlmFilterDF typical price")) %>% 
  ggplot(aes(y=test))+ylab("Model")+xlab("Forecast Error Average")+geom_boxplot(aes(x=error))

grid.arrange(
  result_dlmFilterDF_df         %>% plot_smoothed_dlmFilterDF(),
  result_dlmFilterDF_typical_df %>% plot_smoothed_dlmFilterDF(),
  ncol=1
)
